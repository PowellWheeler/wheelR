...
#'
#' @export
Z2A <- function(Z, SE_Z = as.numeric(NA), CI = 0.95){

  if( (is.vector(Z) && is.vector(SE_Z) && is.vector(CI)) &&  #all the inputs are the correct object: vectors
      (length(Z) == 1 && length(SE_Z) == 1 && length(CI) == 1) && #all the inputs are the correct length: 1
      (is.numeric(Z) && is.numeric(SE_Z) && is.numeric(CI)) && #all the inputs are the correct data class: numeric
